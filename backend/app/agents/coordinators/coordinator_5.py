...
class AgentCoordinator:
    ...
    async def analyze(
        ...
        await agent_output("scheduler", last_content(sch)[:500], detect_handoff(sch))
        detect_handoff(sch)
        logger.info("[调度师] 完成")
        ...
        await agent_output("reviewer", last_content(reviewer)[:500], detect_handoff(reviewer))
        ...
        if not decision.get("signal"):
            logger.warning(f"Trader 解析失败(含重试): {parsed.error[:100]}")
        ...
        return {
            ...
            "source_count": 5,
            "agent_reports": {
                "scheduler": last_content(sch),
                "analyst": last_content(analyst),
                "reviewer": last_content(reviewer),
