...
def get_position() -> str:
    ...
    return f"持仓: {'多头' if pos['side'] == 'long' else '空头'} {pos.get('size', 0)}张 入场${pos.get('entry_price', 0):.2f}"


def get_account_summary() -> str:
    """获取账户概况。"""
    eq, pr, pos = _equity(), _price(), _position()
    margin = float(pos["size"]) * float(pos["entry_price"]) * 0.01 if pos and pos.get("size") else 0.0
...
def get_unrealized_pnl() -> str:
    ...
    pnl, entry, size = float(pos.get("unrealized_pnl", 0)), float(pos.get("entry_price", pr)), float(pos.get("size", 0))
