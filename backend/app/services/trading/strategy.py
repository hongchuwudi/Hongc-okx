...
# 策略协调服务 管理多个策略实例，在每个 tick 周期并行调用它们生成信号， 然后按加权投票汇总为一个最终信号。
class StrategyService:
    ...
    # 并行调用所有策略生成信号，然后加权汇总
    async def analyze(
        self, df: pd.DataFrame, position: dict | None = None, symbol: str = ""
    ) -> AggregatedSignal:
    ...
    async def get_status(self) -> List[dict]:
        return [
            {"name": s.name, "weight": self._weights.get(s.name, 1.0)}
            for s in self._strategies
        ]

    # 返回系统支持的策略列表（静态配置，后续可扩展为动态加载）
    @staticmethod
