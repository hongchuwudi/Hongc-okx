...
const AGENTS = [
  ...
  { id: 'memory', name: '复盘师', dept: '战略研究部', accent: 0x6ee7ff, gx: 5, gy: 11 },
...
export default function TradingFloor({ aiReason, lastUpdate }: { aiReason: string; lastUpdate: number | null }) {
  ...
  useEffect(() => {
    ...
    const map: Record<string, string> = { market: p.agents.market || '', risk: p.agents.risk || '', memory: p.agents.memory || '', trader: p.reason || '' }
