 ...
 */
import type { StatusData, Trade, EquityPoint, AgentStatusResponse } from '../types/dashboard'
...
//
// 获取 Agent 实时状态（5 个 Agent 最新输出 + 事件历史）
export async function fetchAgentStatus(): Promise<AgentStatusResponse> {
  return fetchJson<AgentStatusResponse>(`${BASE}/agents/status`)
}
